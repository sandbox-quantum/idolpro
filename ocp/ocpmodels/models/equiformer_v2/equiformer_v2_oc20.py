...
class EquiformerV2_OC20(BaseModel):
    ...
    def __init__(
        ...
        if self.atom_embedding == 'embedding':
            self.sphere_embedding = nn.Embedding(
                self.max_num_elements, self.sphere_channels_all
            )
        else:
            self.sphere_embedding = nn.Sequential(
                nn.Linear(self.max_num_elements, 2 * self.sphere_channels_all),
                ScaledSiLU(),
                nn.Linear(2 * self.sphere_channels_all, self.sphere_channels_all),
            )
        ...
        for _ in range(self.num_output_heads):
            self.energy_blocks.append(FeedForwardNetwork(
                self.sphere_channels,
                self.ffn_hidden_channels,
                self.num_targets,
                self.lmax_list,
        ...
        if self.regress_forces:
            ...
            for idx in range(self.num_output_heads):
                self.force_blocks.append(SO2EquivariantGraphAttention(
                    self.sphere_channels,
                    self.attn_hidden_channels,
                    self.num_heads,
                    self.attn_alpha_channels,
    ...
    def forward(self, data):
        ...
        for idx in torch.unique(data.data_idx):
            ...
            node_energy = self.energy_blocks[idx](x, inds)
            node_energy = node_energy.embedding.narrow(1, 0, 1)
            node_energies.append(node_energy)

        node_energies = torch.cat(node_energies).squeeze(1)

        output = {}
        output['atomic_energies'] = node_energies
        # energy = torch.zeros(
        #     len(data.natoms),
        #     device=node_energy.device,
        #     dtype=node_energy.dtype,
        # )
        # energy.index_add_(0, data.batch, node_energies)
        energy = scatter(node_energies, data.batch, dim=0)
        ...
        ###############################################################
        if self.regress_forces:
            print("I am regressing forces")
            all_forces = []
            for idx in torch.unique(data.data_idx):
                inds = data.data_idx == idx
                forces = self.force_blocks[idx](
                    x,
                    atomic_numbers,
                    edge_distance,
                    edge_index,
                ...
                forces = forces.view(-1, 3)[inds,...]
                all_forces.append(forces)
            output['forces'] = torch.cat(all_forces).squeeze(1)
