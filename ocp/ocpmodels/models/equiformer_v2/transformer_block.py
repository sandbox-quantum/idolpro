...
class SO2EquivariantGraphAttention(torch.nn.Module):
    ...
    def forward(
        ...
        if inds is None:
            inds = torch.ones(x.embedding.shape[0], dtype=bool)
        ...
        x_source = x.clone()
        x_target = x.clone()
        x_source.embedding = x_source.embedding[inds,...]
        x_target.embedding = x_target.embedding[inds,...]
...
class FeedForwardNetwork(torch.nn.Module):
    ...
    def forward(self, input_embedding, inds=None):
        ...
        if self.use_grid_mlp:
            if self.use_sep_s2_act:
                gating_scalars = self.scalar_mlp(
                    input_embedding.embedding[inds,...].narrow(1, 0, 1)
                )
        else:
        ...
        input_embedding = self.so3_linear_1(input_embedding, inds=inds)
                ...
                input_embedding.embedding = torch.cat(
                    (
                        ...
                        input_embedding.embedding.narrow(
                            1, 1, input_embedding.embedding[inds,...].shape[1] - 1
                        ),
                    ),
                    dim=1,
                )
