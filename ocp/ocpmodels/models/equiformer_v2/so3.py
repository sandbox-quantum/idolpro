...
class SO3_Embedding:
    ...
    def to_grid(self, SO3_grid, lmax=-1, inds=None):
        if not inds:
            inds = torch.ones(self.embedding.shape, dtype=bool)
        if lmax == -1:
            lmax = max(self.lmax_list)
        ...
        for i in range(self.num_resolutions):
            num_coefficients = int((self.lmax_list[i] + 1) ** 2)
            if self.num_resolutions == 1:
                x_res = self.embedding[inds,...]
            else:
                x_res = self.embedding[
                    inds, offset : offset + num_coefficients
                ].contiguous()
...
class SO3_Linear(torch.nn.Module):
    ...
    def forward(self, input_embedding, output_scale=None, inds=None):

        if not inds:
            inds = torch.ones(input_embedding.embedding.shape[0], dtype=bool)
        out = []
        for lval in range(self.lmax + 1):
            start_idx = lval**2
            length = 2 * lval + 1
            features = input_embedding.embedding[inds,...].narrow(1, start_idx, length)
...
class SO3_LinearV2(torch.nn.Module):
    ...
    def forward(self, input_embedding, inds=None):
        ...
        out = torch.einsum(
            "bmi, moi -> bmo", input_embedding.embedding[inds,...], weight
        )  # [N, (L_max + 1) ** 2, C_out]
