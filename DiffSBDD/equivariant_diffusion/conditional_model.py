...
class ConditionalDDPM(EnVariationalDiffusion):
    ...
    def forward(self, ligand, pocket, return_info=True):
        """
        ...
        # setting to 0 until we figure this out.
        log_pN = torch.tensor([0.])
        info = {
        ...
        loss_terms = (delta_log_px, error_t_lig, torch.tensor(0.0), SNR_weight,
                      ...
                      t_int.squeeze(), xh_lig_hat, eps_t_lig, net_out_lig)
    ...
    # @torch.no_grad()
    def sample_given_pocket(self, pocket, lig_mask, return_frames=1,
                            timesteps=None, z_lig_0=None, t_interval=None):
        """
        ...
        if z_lig_0 is None:
            ...
            z_lig = z_lig_0.clone()
            xh_pocket = xh0_pocket.detach().clone()
            z_lig[:, :self.n_dims], xh_pocket[:, :self.n_dims] = \
                self.remove_mean_batch(z_lig[:, :self.n_dims],
                                    xh0_pocket[:, :self.n_dims],
        ...
        for s in reversed(range(t_interval[1], t_interval[0])):
        ...
        return out_lig.squeeze(0), out_pocket.squeeze(0), z_lig

    #@torch.no_grad()
    def inpaint(self, ligand, pocket, lig_fixed, resamplings=1, return_frames=1,
            timesteps=None, z_lig_0=None, center='pocket', t_interval=None):
        """
        ...
        #need to change to not get z_lig known from a latent
        if z_lig_0 is None:
            mu_lig_x = mean_known
            mu_lig_h = torch.zeros((n_samples, self.atom_nf), device=device)
