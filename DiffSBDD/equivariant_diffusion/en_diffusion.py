...
class EnVariationalDiffusion(nn.Module):
    ...
    def __init__(
            ...
            n_dims: int, size_histogram: Dict = None,
    ...
    # @torch.no_grad()
    def inpaint(self, ligand, pocket, lig_fixed, pocket_fixed, resamplings=2,
                jump_length=1, return_frames=1, timesteps=None, z_lig_0=None, z_pocket_0=None, t_interval=None):
        """
        ...
        if z_lig_0 is None:
            ...
            z_lig = z_lig_0.clone()
            z_pocket = z_pocket_0.clone()
            z_x_0 = self.remove_mean_batch(
                torch.cat((z_lig[:, :self.n_dims], z_pocket[:, :self.n_dims]), dim=0),
                torch.cat((ligand['mask'], pocket['mask'])))
            z_lig[:, :self.n_dims] = z_x_0[:len(z_lig_0), :self.n_dims]
            z_pocket[:, :self.n_dims] = z_x_0[len(z_lig_0):, :self.n_dims]
        ...
        if t_interval is not None:
            schedule = self.get_repaint_schedule(resamplings, jump_length, t_interval[0] - t_interval[1])
            s = t_interval[0] - 1
        else:
        ...
        # remove frame dimension if only the final molecule is returned
        return out_lig.squeeze(0), out_pocket.squeeze(0), z_lig, z_pocket
    ...
    def assert_mean_zero_with_mask(x, node_mask, eps=1e-10):
        ...
        assert rel_error < 1e-2, f'Mean i \s not zero, relative_error {rel_error}'
