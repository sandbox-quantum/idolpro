...
class LigandPocketDDPM(pl.LightningModule):
    def __init__(
        ...
        # # turn into namespace
        loss_params = vars(loss_params)
        eval_params = vars(eval_params)
        diffusion_params = vars(diffusion_params)
        egnn_params = vars(egnn_params)
        ...
        if 'atom_hist' in self.dataset_info:
        ...
        net_dynamics = EGNNDynamics(
            ...
            edge_cutoff_ligand=egnn_params['edge_cutoff_ligand'],
            edge_cutoff_pocket=egnn_params['edge_cutoff_pocket'],
            edge_cutoff_interaction=egnn_params['edge_cutoff_interaction'],
        ...
        if 'lennard_jones_rm' in self.dataset_info:
    ...
    def forward(self, data):
        ...
        kl_prior, log_pN, t_int, xh_lig_hat, true_noise, pred_noise, info = \
        ...
        if self.loss_type == 'l2' and self.training:
            ...
            actual_ligand_size = torch.where(actual_ligand_size == 0.0, 1.0, actual_ligand_size)
            # normalize loss_t
            ...
            if error_t_pocket.shape[0] != 0:
                loss_t += 0.5 * error_t_pocket
                loss_0_x_pocket = loss_0_x_pocket / (self.x_dims * pocket['size'])
                loss_0 += loss_0_x_pocket
        ...
        info["true_noise"] = true_noise
        info["pred_noise"] = pred_noise
    ...
    def prepare_pocket(self, biopython_residues, repeats=1):
        ...
        if self.pocket_representation == 'CA':
            ...
            pocket_types = torch.tensor(
                [self.pocket_type_encoder[three_to_index(res.get_resname())]
    ...
    def generate_ligands(self, pdb_file, n_samples, pocket_ids=None,
        ...
        # assert (pocket_ids is None) ^ (ref_ligand is None) ^ (ref_ligand_sdf is None)
        ...
        if pocket_ids is not None:
            ...
            residues = utils.get_pocket_from_sdf(pdb_struct, ref_ligand)
        ...
        if type(self.ddpm) == EnVariationalDiffusion:
            ...
            # TODO: can split this script into two here to pass in latent variables
            ...
            xh_lig, xh_pocket, lig_mask, pocket_mask, z_lig_0 = self.ddpm.inpaint(
